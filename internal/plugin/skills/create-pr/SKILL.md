...
1. **Check if on main branch**:
   ...
   - If current branch is `main` or `master` (or matches the remote's default branch):
     - Analyze staged/unstaged changes to determine branch name
     - Create descriptive branch name: `<type>/<brief-description>`
       - Examples: `feat/workflow-metrics`, `fix/memory-leak`, `docs/api-reference`
